...
namespace com.bhambhoo.fairludo
{
    ...
    public class PlayersManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            //int i=Convert.ToInt32(Players[Player])
            //AIGameManager.Instance.tokensint[Players]
        }
        ...
        public bool ChangePlayerType(PlayerAI player, Constants.PlayerType newType)
        {
        ...
        public void Initialize(int numberOfPlayers, Constants.MatchType matchType)
        {
            ...
            Players.Add(new PlayerAI(1, Constants.PlayerType.LocalPlayer));
            ...
            Players.Add(new PlayerAI(3, restPlayerTypes));
            if (numberOfPlayers > 2)
            {
                Players.Add(new PlayerAI(2, restPlayerTypes));
                if (numberOfPlayers > 3)
                {
                    Players.Add(new PlayerAI(4, restPlayerTypes));
                }
            }
        }

        public static PlayerAI GetPlayer(byte playerIndex)
        {
            foreach (PlayerAI onePlayer in Players)
            {
        ...
        /// <returns></returns>
        public static List<PlayerToken> GetTokensAt(int givenWaypointIndex, PlayerAI withRespectTo)
        {
            tokensAt.Clear();
    ...
    public class PlayerAI
    {
        ...
        public PlayerAI(byte playerIndex, Constants.PlayerType playerType)
        {
            ...
            foreach (Transform oneBase in Constants.Instance.GetBases(playerIndex))
            {
                if(playerIndex==1)
                {
                    playerTokens[i] = GameObject.Instantiate(Constants.Instance.bluePlayerToken).GetComponent<PlayerToken>().Initialize(this, oneBase, Constants.Instance.GetplayerToken(playerIndex));
                    i++;
                }
                if (playerIndex == 2)
                {
                    playerTokens[i] = GameObject.Instantiate(Constants.Instance.redPlayerToken).GetComponent<PlayerToken>().Initialize(this, oneBase, Constants.Instance.GetplayerToken(playerIndex));
                    i++;
                }
                if (playerIndex == 3)
                {
                    playerTokens[i] = GameObject.Instantiate(Constants.Instance.greenPlayerToken).GetComponent<PlayerToken>().Initialize(this, oneBase, Constants.Instance.GetplayerToken(playerIndex));
                    i++;
                }
                if (playerIndex == 4)
                {
                    playerTokens[i] = GameObject.Instantiate(Constants.Instance.yellowPlayerToken).GetComponent<PlayerToken>().Initialize(this, oneBase, Constants.Instance.GetplayerToken(playerIndex));
                    i++;
                }
